package com.example.readstoryapp.database;
...
public class DatabaseStory extends SQLiteOpenHelper {
    ...
    //Name database
    private static String DATABASE_NAME = "readstory";
    //table account
    private static String TABLE_ACCOUNT = "account";
    private static String ID_ACCOUNT = "idaccount";
    private static String USERNAME = "username";
    private static String PASSWORD = "password";
    private static String DECENTRALIZATION = "decentralization";
    private static String EMAIL = "email";
    private static int VERSION = 1;

    //table story
    private static String TABLE_STORY = "story";
    private static String ID_STORY = "idstory";
    ...
    private String SQLQuery3 = "INSERT INTO account VAlUES (null,'luong','luong','luong@gmail.com',1)";
    ...
    //Method add account to database
    public void addAccount(Account account){
        SQLiteDatabase database = this.getWritableDatabase();

        //Let insert through ContentValues
        ContentValues values = new ContentValues();
        values.put(USERNAME, account.getmUsername());
        values.put(PASSWORD, account.getmPassword());
        values.put(EMAIL, account.getmEmail());
        values.put(DECENTRALIZATION, account.getmDecentralization());
    ...
    //Get story lastest
    public Cursor getDataStoryLastest(){
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor res = database.rawQuery("SELECT * FROM " + TABLE_STORY + " ORDER BY " + ID_STORY + " DESC LIMIT 3", null);
        return res;
    }
    ...
    public Cursor getAllStory(){
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor res = database.rawQuery("SELECT * FROM " + TABLE_STORY, null);
        return res;
    }
    ...
    public  void addStory(Story story){
        ...
        values.put(ID_ACCOUNT, story.getId_user());

        database.insert(TABLE_STORY, null, values);
        database.close();
    }
    ...
    public int deleteStory(int id){
        ...
        int res = database.delete(TABLE_STORY, ID_STORY + " = " + id, null);
        return res;
    }

    //Update story info
    public void updateStory(Story story, int id){
        SQLiteDatabase database = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(ID_STORY, story.getId());
        values.put(NAME_STORY, story.getNameStory());
        values.put(CONTENT, story.getContent());
        values.put(IMAGE, story.getImage());
        values.put(ID_ACCOUNT, story.getId_user());

        database.update(TABLE_STORY, values, ID_STORY + " = " + id, new String[]{String.valueOf(id)});
        database.close();
    }
}
